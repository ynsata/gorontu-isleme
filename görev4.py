...
gonderim_araligi = 0.5  # 1 saniyede bir gönder
...
aktif_hedef_id = None
atis_yapildi = False
MERKEZ_TOLERANS = 30  # Merkezden ±50px
...
while True:
    ...
    for track in results[0].boxes:
        ...
        if class_name != "kirmizi":
            continue  #  Yalnızca kırmızı balonları işleriz
        ...
        print(f" KIRMIZI Balon - ID:{track_id}, Güven: {confidence:.2f}, Konum: ({cx}, {cy})")
        ids.append(track_id)
        kirmizi_balonlar.append({
            "track_id": track_id,
        ...
        label = f"KIRMIZI ID:{track_id} {confidence:.2f}"
    ...
    if kirmizi_balonlar:
        ...
        if aktif_hedef_id not in ids:
            aktif_hedef_id = ids[0]  # En küçük ID'li kırmızı balon
            print(f" Yeni hedef kilitlendi: ID {aktif_hedef_id}")

        hedef = next((d for d in kirmizi_balonlar if d["track_id"] == aktif_hedef_id), None)
        ...
        aktif_hedef_id = None
        atis_yapildi = False
        print(" Sahnede KIRMIZI balon yok.")

    cv2.imshow('Kirmizi Balon Takibi', frame)
